<?php
	...
	// Temporar pass so that we can access data page [potential security issue]
	$_SESSION['userObj'] = 'temporary pass';
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		if ($lg && $userID && $sp && $tn && $ct && $st) {
			...
			// Need logic to redirect to player page
...
<div id="roleQuestion">
	<h3> Are you a player or manager?</h3>
	<p class="tip">TIP: You can become a manager in the future if you are not currently one.</p>
	<select name="roleOption" id="roleOption" onchange="REGISTRATION.showForm(this.value)">
...
<form action="oauth_welcome.php" method="post" id="OAFirstTeamForm">
